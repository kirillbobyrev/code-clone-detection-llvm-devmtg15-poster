...
\tikzstyle{sensor}=[draw, fill=blue!20, text width=5em, text centered,
...
\begin{tikzpicture}
    \node (Analyzed_Sources) [sensor, fill=]  {Analyzed sources};
    \path (Analyzed_Sources.east)+(3.0,0.0) node (scan-build)[sensor, fill=orange!30]
          {scan-build};
    \path (scan-build.east)+(3.0, 0.0) node (fake-compiler)[sensor] {Fake compiler};
    \path (fake-compiler.east)+(4.0, 2.0) node (compiler)[sensor, fill=gray!20] 
    	  {Compiler};
    \path (fake-compiler.east)+(4.0, -2.0) node (static-analyzer)[sensor, fill=gray!20]
          {Static Analyzer};
    \path (compiler.east)+(4.0, 0.0) node (object-code)
          [sensor, fill=black!60, text=white] {Object code};
    \path (static-analyzer.east)+(4.0, 0.0) node (report)
          [sensor, fill=red!40] {SA Report};
